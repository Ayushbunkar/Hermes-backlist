...
def add_or_update_project(
    ...
    with _connect(db_path) as conn:
        sets = ["niche = ?", "status = ?", "scan_interval_minutes = ?"]
...
def set_project_group(
    ...
    *,
    ...
    with _connect(db_path) as conn:
        ...
        sets = ["telegram_group_id = ?", "telegram_group_name = ?"]
    ...
    with _connect(db_path) as conn:
        rows = conn.execute(
            ...
            WHERE h.project_id = ?
              ...
              AND s.first_seen_at <= datetime('now', ? || ' days')
