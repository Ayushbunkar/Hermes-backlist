...
def _run_column_migrations(conn: psycopg2.extensions.connection) -> None:
    for table, columns in _COLUMN_MIGRATIONS.items():
        try:
            existing = {row["name"] for row in conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute(f"PRAGMA table_info({table})")}
...
def init_whitelist_db(db_path: str = DEFAULT_DB_PATH) -> None:
    ...
    with _connect(db_path) as conn:
        conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).executescript(_NEW_SCHEMA)
...
def upsert_project(project_url: str, niche: str, name: str = "", db_path: str = DEFAULT_DB_PATH) -> int:
    ...
    with _connect(db_path) as conn:
        ...
        conn.commit()
        row = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute("SELECT id FROM projects WHERE project_url=%s", (project_url.strip(),)).fetchone()
    return int(row["id"])
...
def set_site_status(whitelist_site_id: int, status: str, db_path: str = DEFAULT_DB_PATH) -> None:
    ...
    init_whitelist_db(db_path)
    with _connect(db_path) as conn:
        conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute("UPDATE whitelist_sites SET status=%s WHERE id=%s", (status, whitelist_site_id))
        conn.commit()
...
def mark_seen_batch(project_id: int, url_keys: list[str], db_path: str = DEFAULT_DB_PATH) -> None:
    ...
    init_whitelist_db(db_path)
    with _connect(db_path) as conn:
        conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).executemany(
...
def add_or_update_project(
    ...
    with _connect(db_path) as conn:
        ...
        params.append(pid)
        conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute(f"UPDATE projects SET {', '.join(sets)} WHERE id=%s", params)
        conn.commit()
    return pid
...
def set_project_group(
    ...
    *,
    ...
    with _connect(db_path) as conn:
        ...
        if not row:
            raise ValueError(f"project not found: {project_url}")
        dup = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute(
        ...
        conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute(f"UPDATE projects SET {', '.join(sets)} WHERE project_url=%s", params)
        conn.commit()
...
def get_project(project_url: str, db_path: str = DEFAULT_DB_PATH) -> dict | None:
    init_whitelist_db(db_path)
    with _connect(db_path) as conn:
        row = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute("SELECT * FROM projects WHERE project_url=%s", (project_url.strip(),)).fetchone()
...
def list_projects(db_path: str = DEFAULT_DB_PATH) -> list[dict]:
    init_whitelist_db(db_path)
    with _connect(db_path) as conn:
        rows = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute("SELECT * FROM projects ORDER BY created_at").fetchall()
    return [dict(r) for r in rows]
...
def get_active_projects(db_path: str = DEFAULT_DB_PATH) -> list[dict]:
    init_whitelist_db(db_path)
    with _connect(db_path) as conn:
        rows = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute("SELECT * FROM projects WHERE status = 'active' ORDER BY created_at").fetchall()
    return [dict(r) for r in rows]
...
def delete_project(project_url: str, db_path: str = DEFAULT_DB_PATH) -> None:
    ...
    with _connect(db_path) as conn:
        ...
        if not row:
            raise ValueError(f"project not found: {project_url}")
        pid = row["id"]
        conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute("DELETE FROM harvest_leads WHERE project_id=%s", (pid,))
        conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute("DELETE FROM seen_opportunities WHERE project_id=%s", (pid,))
        conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute(
            "DELETE FROM site_score_history WHERE whitelist_site_id IN "
            "(SELECT id FROM whitelist_sites WHERE project_id=%s)",
            (pid,),
        )
        conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute("DELETE FROM whitelist_sites WHERE project_id=%s", (pid,))
        conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute("DELETE FROM projects WHERE id=%s", (pid,))
        conn.commit()
...
def purge_harvest_pipeline(
    ...
    with _connect(db_path) as conn:
        if project_id is not None:
            cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute("DELETE FROM harvest_leads WHERE project_id=%s", (project_id,))
            counts["harvest_leads"] = cur.rowcount
            cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute("DELETE FROM seen_opportunities WHERE project_id=%s", (project_id,))
            counts["seen_opportunities"] = cur.rowcount
        else:
            cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute("DELETE FROM harvest_leads")
            counts["harvest_leads"] = cur.rowcount
            cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute("DELETE FROM seen_opportunities")
            counts["seen_opportunities"] = cur.rowcount
        conn.commit()
    return counts
...
def refresh_scan_priorities(project_id: int, db_path: str = DEFAULT_DB_PATH) -> int:
    ...
    updated = 0
    with _connect(db_path) as conn:
        sites = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute(
...
def revive_lead(project_id: int, url_key: str, db_path: str = DEFAULT_DB_PATH) -> bool:
    ...
    key = str(url_key or "").lower().rstrip("/")
    with _connect(db_path) as conn:
        seen = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor).execute(
            """
