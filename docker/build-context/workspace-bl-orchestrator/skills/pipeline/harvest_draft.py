...
def invoke_ink(project: dict, run_dir: str, manifest_path: str, *, log_fn: Callable[[str], None] | None = None) -> bool:
    ...
    cmd = [
        ...
        "--agent", "bl-content", "--message", task,
        "--timeout", str(INK_TIMEOUT), "--json",
    ]
    plog_verbose("draft", "ink_invoke", run_dir=run_dir, manifest=manifest_path)
    try:
        proc = subprocess.run(cmd, capture_output=True, text=True, timeout=INK_TIMEOUT + 60)
    except subprocess.TimeoutExpired:
        log("draft: Ink invocation timed out")
        return False
    except FileNotFoundError:
        log("draft: ERROR 'openclaw' CLI not found on PATH")
        return False
    if proc.returncode != 0:
        log(f"draft: Ink returned {proc.returncode}: {proc.stderr[:300]}")
